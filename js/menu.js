    ...
    $.fn.lavaLamp = function(o) {
        ...
        return this.each(function(index) {
            ...
            function setCurr(el) {
                ...
                //$back.css({ "top": el.offsetTop + "px", "width": el.offsetWidth + "px" });
                curr = el;
            };
...
jQuery(function() {
    ...
    // base rules
    $('ul ul', '#menu').css({display: 'none', left: 0,width:'110px'});
    $('li', '#menu').hover(
        function(){
            var ul = $('ul:first', this);
            $('span', ul).css('color', '#fff');
            if (ul.length){
                if (!ul[0].wid){
                    ul[0].wid = ul.width();
                    ul[0].hei = ul.height();
    ...
    //    $('ul ul a ', '#menu').css('color', '#bc1b4e').hover(
    //        function () { $(this).animate({ color: '#00a2e6' }, 100); },
    //        function () { $(this).animate({ color: '#bc1b4e' }, 100); }
    //    );
    //}
    if (!($.browser.msie && $.browser.version.substr(0, 1) == '6')) {
        $('ul ul a ', '#menu').hover(
